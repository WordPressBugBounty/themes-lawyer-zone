 ...
 * @return void
 */
if ( ! function_exists( 'lawyer_zone_default_slider' ) ) :
	function lawyer_zone_default_slider() {
...
function lawyer_zone_slider_from_page() {
	$lawyer_zone_customizer_all_values = lawyer_zone_get_theme_options();

	$lawyer_zone_slides_data               = json_decode( $lawyer_zone_customizer_all_values['lawyer-zone-slides-data'] );
	$lawyer_zone_feature_slider_text_align = $lawyer_zone_customizer_all_values['lawyer-zone-feature-slider-text-align'];

	$lawyer_zone_feature_slider_enable_animation = $lawyer_zone_customizer_all_values['lawyer-zone-feature-slider-enable-animation'];
	$lawyer_zone_feature_slider_image_only       = $lawyer_zone_customizer_all_values['lawyer-zone-feature-slider-display-title'];
	$lawyer_zone_feature_slider_image_excerpt    = $lawyer_zone_customizer_all_values['lawyer-zone-feature-slider-display-excerpt'];
	$lawyer_zone_fs_image_display_options        = $lawyer_zone_customizer_all_values['lawyer-zone-fs-image-display-options'];

	$post_in           = array();
	$slides_other_data = array();
	if ( is_array( $lawyer_zone_slides_data ) ) {
		foreach ( $lawyer_zone_slides_data as $slides_data ) {
	...
	if ( ! empty( $post_in ) && is_array( $post_in ) ) :
		$lawyer_zone_child_page_args = array(
			'post__in'       => $post_in,
			'orderby'        => 'post__in',
		...
		$slider_query                = new WP_Query( $lawyer_zone_child_page_args );
		/*The Loop*/
		if ( $slider_query->have_posts() ) :
			?>
			<div class="image-slider-wrapper home-fullscreen <?php echo esc_attr( $lawyer_zone_fs_image_display_options ); ?>">
				<div class="featured-slider">
					<?php
					$slider_index = 1;
					$text_align   = '';
					...
					$bg_image_style = '';
					if ( 'alternate' != $lawyer_zone_feature_slider_text_align ) {
						$text_align = $lawyer_zone_feature_slider_text_align;
					}
					if ( 1 == $lawyer_zone_feature_slider_enable_animation ) {
					...
					while ( $slider_query->have_posts() ) :
						...
						if ( 'full-screen-bg' == $lawyer_zone_fs_image_display_options ) {
						...
						<div class="item" style="<?php echo $bg_image_style; ?>">
							...
							<div class="slider-content <?php echo esc_attr( $text_align ); ?>">
								<div class="container">
									<?php
									if ( 1 == $lawyer_zone_feature_slider_image_only ) {
										?>
										<div class="banner-title <?php echo esc_attr( $animation1 ); ?>"><?php the_title(); ?></div>
										<?php
									}
									if ( 1 == $lawyer_zone_feature_slider_image_excerpt ) {
										?>
...
if ( ! function_exists( 'lawyer_zone_feature_slider' ) ) :

	function lawyer_zone_feature_slider() {
		lawyer_zone_slider_from_page();
	}
endif;
add_action( 'lawyer_zone_action_feature_slider', 'lawyer_zone_feature_slider', 0 );
